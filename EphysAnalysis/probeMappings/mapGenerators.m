...
% save the probe map for ASSY77 H2 probe (left shank broken)
connected(33:64) = false;
save(fullfile(getenv('OBSDATADIR'), 'ephys', 'channelMaps', 'kilosort', 'ASSY77_H2Probe_QZ2.mat'), ...
    'chanMap','channelNum_OpenEphys','connected', 'xcoords', 'ycoords', 'kcoords', 'chanMap0ind', 'fs')
...
% save the probe map for ASSY77 H2 probe (left shank broken)
connected(33:64) = false;
save(fullfile('D:\DCN_project\Ephys\ASSY_KS1ConfigFile', 'ASSY77_H2_2.mat'), ...
    'chanMap','channelNum_OpenEphys','connected', 'xcoords', 'ycoords', 'kcoords', 'chanMap0ind', 'fs')


%% NeuroCambridge ASSY-H6-QianyunVersion (32 channel per shank*2)

channelNum_OpenEphys = [33 38 44 40 46 42 41 45 39 43 37 49 63 34 61 36 60 48 58 35 56 54 52 50 47 51 53 55 57 59 62 64 11 1 9 3 7 6 5 8 4 10 2 12 28 14 26 16 24 17 19 13 21 15 32 30 18 29 31 22 20 23 25 27];  


% get probe physical layout
shankSeparation = 150; % The left shank and right shank are separated by 250um.
y = [387.5:-12.5:0]'; % y coordinates for both shanks.
x = [repmat([22.5 0], 1, 16), repmat([shankSeparation+22.5, shankSeparation], 1, 16)]'; % x coordinates for left shank and right shank.
sitaLocations_shank1 = [x(1:32, 1) y];
siteLocations_shank2 = [x(33:64, 1) y];
siteLocations = [sitaLocations_shank1; siteLocations_shank2]; % xy coordinates for both shanks. dim: Nchannels*Nshank
...
% save the probe map for ASSY77 H6 probe
save(fullfile(getenv('OBSDATADIR'), 'ephys', 'channelMaps', 'kilosort', 'ASSY77_H6Probe_QZ.mat'), ...
    'chanMap','channelNum_OpenEphys','connected', 'xcoords', 'ycoords', 'kcoords', 'chanMap0ind', 'fs')


% save the probe map for ASSY77 H6 probe
save(fullfile('D:\DCN_project\Ephys\ASSY_H6_KS1ConfigFile', 'ASSY77_H6Probe_QZ.mat'), ...
    'chanMap','channelNum_OpenEphys','connected', 'xcoords', 'ycoords', 'kcoords', 'chanMap0ind', 'fs')


%% Neuronexus A1x32-Poly3-10mm-50-177

channelNum_OpenEphys = [1, 27, 17, 5, 31, 16, 4, 22, 32, 13, 18, 3, 6, 28, 19, 12, 23, 14, 11, 21, 30, 15, 26, 9, 7, 29, 25, 2, 24, 10, 8, 20];


% get probe physical layout
y = 550; % y coordinates for both shanks.
for i = 1:10
    y = [y; repmat(550-i*50, 3, 1)];
end
y = [y; 0];
x = [50; repmat([0; 50; 100], 10, 1); 50];
siteLocations = [x,y];
...
% save the probe map for NeuroNexus A1x32-poly3-10mm-50-177
save(fullfile(getenv('OBSDATADIR'), 'ephys', 'channelMaps', 'kilosort', 'NeuroNexus_A1x32_Poly3.mat'), ...
    'chanMap','channelNum_OpenEphys','connected', 'xcoords', 'ycoords', 'kcoords', 'chanMap0ind', 'fs');

%% Neuronexus A1x32-Linear-10mm-50-177

channelNum_OpenEphys = [1, 17, 16, 32, 3, 19, 14, 30, 9, 25, 10, 20, 8, 24, 2, 29, 7, 26, 15, 21, 11, 23, 12, 28, 6, 18, 13, 22, 5, 27, 4, 31];
