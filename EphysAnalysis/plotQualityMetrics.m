...
info.timestamps = info.convertedEphysTimestamps;
...
folder = fullfile(rootFolder, 'Figures', 'Ephys', 'QualityMetrics');  % figures will be saved here
...
% load data
data = memmapfile(fullfile(info.ephysFolder, [info.fileNameBase '_CHs.dat']), ...
...
for c = unitInds
    ...
    figure('Name', sprintf('%s cell %i', session, unit_ids(c)), 'Visible', showFigures, ...
        'Color', 'white', 'units', 'normalized', 'Position', get(0,'ScreenSize')); hold on
...
%     shankInds = find(abs(xcoords-xcoords(bestChannels(c))) < 50)';  % temporary hack until kcoords is accurate
    ...
    set(gca, 'YColor', cellColors(c,:), 'YLim', [min(percentiles*0.5), max(percentiles)*1.2], 'Box', 'off');
