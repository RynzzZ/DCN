function traces = showChannelsTimeChunk(session, startTimes, stopTimes, varargin) 
...
s.dataOnly = false;               % this toggles the channel trace plot // set it to true to inhibit the plotting
...
if strcmp(s.selectedChannels, 'all')
    selectedChannels = channelNum_OpenEphys;
else
...
if s.showSortedSpikes
    ...
    colors = parula(length(selectedChannels));
end
...
% plot channels at specific time
startInd = find(info.convertedEphysTimestamps>startTimes,1,'first');
stopInd = find(info.convertedEphysTimestamps<stopTimes,1,'last');
...
if ~s.dataOnly
    ...
    for j = 1:length(selectedChannels)
        offset = (length(channelNum_OpenEphys) - j)*range(s.yLims);
